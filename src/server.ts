...
(async () => {
  ...
  app.get("/", async (req: Request, res: Response) => {
    res.status(400).send("try POST /filteredimage, with data {image_url:'Put the url'}")
  });


  app.post("/filteredimage", async (req: Request, res: Response) => {

    if (!req.body.image_url) res.status(400).send("try POST /filteredimage, with data {image_url:'Put the url'}")
